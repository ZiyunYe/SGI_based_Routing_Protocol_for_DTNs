...
public class SimScenario implements Serializable {
	...
	protected SimScenario() {
		...
		//CS218
		this.hostList = new ArrayList<Long>();
	...
	public void convertHostID() {
		...
		try {
			...
			for(int i=0; i<hosts.size(); i++) {
				content = hosts.get(i).getID() + "/" + hosts.get(i).getAddress();
				bw.write(content);
				bw.newLine();
			}
