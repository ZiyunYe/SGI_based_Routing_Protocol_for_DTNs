...
public class DTNHost implements Comparable<DTNHost> {
	...
	//private HashMap<Long, List<Coord>> mobilityPattern;
	//private List<Coord> wayPoints;
	...
	public DTNHost(List<MessageListener> msgLs,
			...
			MovementModel mmProto, MessageRouter mRouterProto, long ID) {
		this.ID = ID; //CS218
		this.comBus = comBus;
	...
	private boolean setNextWaypoint() {
		if (path == null) {
			...
			try {
				...
				while ((sCurrentLine = br.readLine()) != null) {
					...
					if(this.ID == hostID) { 
						path = new Path();
						String[] points = sCurrentLine.split(":")[1].substring(1).split(",");
						
						for(int i=0; i < points.length; i++) {
							String point = points[i].split("'")[1];
							double pointX = Double.parseDouble(point.split("/")[0]);
							double pointY = Double.parseDouble(point.split("/")[1]);
							Coord wayPoint = new Coord(pointX, pointY);
		...
		*/
		...
		//this.speed = rng.nextInt(10) + 1;
		/*
		int dontMove = rng.nextInt(2);
		if(dontMove == 0){
			this.destination = path.getNextWaypoint();
			this.speed = 1;
		}else {
			this.destination = path.getNextWaypoint(0);
			this.speed = 0;
			return false;
		}
		*/
