...
public class MessageEventGenerator implements EventQueue {
	...
	private HashMap<Integer, LinkedList<Integer>> friendMap;
	private HashMap<Long, Integer> hostAddrMap;
	...
	public MessageEventGenerator(Settings s){
		...
		//CS218
		/* Make a friend list */
		friendMap = new HashMap<Integer, LinkedList<Integer>>();
		hostAddrMap = new HashMap<Long, Integer>();
		
		BufferedReader br = null;
		
		try {
			String line;
			br = new BufferedReader(new FileReader("hostID"));
			
			while ((line = br.readLine()) != null) {
				hostAddrMap.put(Long.parseLong(line.split("/")[0]), Integer.parseInt(line.split("/")[1]));
			}
		...
		try {
			String sCurrentLine;
			br = new BufferedReader(new FileReader("friend_list"));

			while ((sCurrentLine = br.readLine()) != null) {
				destinations = new LinkedList<Integer>();
				long hostFrom = Long.parseLong(sCurrentLine.split(":")[0]);
				long hostTo = Long.parseLong(sCurrentLine.split(":")[1]);
				int hostFromAddr = hostAddrMap.get(hostFrom);
				int hostToAddr = hostAddrMap.get(hostTo);
				
				if(friendMap.containsKey(hostFromAddr)) {
					destinations = friendMap.get(hostFromAddr);
					destinations.add(hostToAddr);
					friendMap.put(hostFromAddr, destinations);
					//System.out.println("e: " + hostFromAddr + " - " + hostToAddr);
				} else {
					destinations.add(hostToAddr);
					friendMap.put(hostFromAddr, destinations);
					//System.out.println("n: " + hostFromAddr + " - " + hostToAddr);
				}
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	*/
	...
	//CS218
	protected int drawFromAddress() {
		int from;
		do {
			from = drawHostAddress(this.hostRange); 
		} while (!friendMap.containsKey(from));
		return from;
	}
	//CS218
	protected int drawToAddress(int from) {
		int destinationHosts = friendMap.get(from).size();
		int to = friendMap.get(from).get(rng.nextInt(destinationHosts));
		return to;
	}
	...
	public ExternalEvent nextEvent() {
		...
		from = drawFromAddress();	
		to = drawToAddress(from); //CS218
